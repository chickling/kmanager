...
public abstract class OffsetGetter {
	...
	protected KafkaConsumerGroupService kafkaConsumerGroupService = null;
	...
	protected List<OffsetInfo> getOffsetInfoCommittedToBroker(String group, List<String> topics) {
		...
		Map<String, List<PartitionAssignmentState>> groupAssignment = this.kafkaConsumerGroupService
				.describeGroup(group);
		List<PartitionAssignmentState> partitionAssignments = groupAssignment.get(group);
		// TODO dead group
			...
			if (!topics.isEmpty()) {
				if (topics.contains(partitionAssignment.getTopic().orElse("-"))) {
					offsets.add(new OffsetInfo(group, (String) partitionAssignment.getTopic().orElse("-"),
							(Integer) partitionAssignment.getPartition().orElse(-1),
							(Long) partitionAssignment.getOffset().orElse(-1l),
							(Long) partitionAssignment.getLogEndOffset().orElse(-1l),
							(String) partitionAssignment.getConsumerId().orElse("-"), -1L, -1L,
	...
	private List<KafkaInfo> mapConsumersToKafkaInfo(List<String> consumers, String topic, boolean belongZK)
			throws Exception {
		List<KafkaInfo> kafkaInfos = new ArrayList<KafkaInfo>();
		List<String> topics = new ArrayList<String>();
		topics.add(topic);

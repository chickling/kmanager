						...
						false, new JMXExecutor() {
							...
							public void doWithConnection(MBeanServerConnection mBeanServerConnection) {
								...
								if (result.containsKey("BytesInPerSec")) {
									result.put("BytesInPerSec", merge(result.get("BytesInPerSec"),
											metrics.getBytesInPerSec(mBeanServerConnection, Optional.empty())));
								} else {
								...
								if (result.containsKey("BytesOutPerSec")) {
									result.put("BytesOutPerSec", merge(result.get("BytesOutPerSec"),
											metrics.getBytesOutPerSec(mBeanServerConnection, Optional.empty())));
								} else {
								...
								if (result.containsKey("BytesRejectedPerSec")) {
									result.put("BytesRejectedPerSec", merge(result.get("BytesRejectedPerSec"),
											metrics.getBytesRejectedPerSec(mBeanServerConnection, Optional.empty())));
								} else {
								...
								if (result.containsKey("FailedFetchRequestsPerSec")) {
									result.put("FailedFetchRequestsPerSec",
											merge(result.get("FailedFetchRequestsPerSec"),
													metrics.getFailedFetchRequestsPerSec(mBeanServerConnection,
															Optional.empty())));
								} else {
									result.put("FailedFetchRequestsPerSec", metrics
											.getFailedFetchRequestsPerSec(mBeanServerConnection, Optional.empty()));
								}
								if (result.containsKey("FailedProduceRequestsPerSec")) {
									result.put("FailedProduceRequestsPerSec",
											merge(result.get("FailedProduceRequestsPerSec"),
													metrics.getFailedProduceRequestsPerSec(mBeanServerConnection,
															Optional.empty())));
								} else {
									result.put("FailedProduceRequestsPerSec", metrics
				...
				if (bid.equals(jmxArr[0])) {
					...
					kafkaJMX.doWithConnection(jmxArr[1], Integer.parseInt(jmxArr[2]), Optional.of(""), Optional.of(""),
							false, new JMXExecutor() {
								...
								public void doWithConnection(MBeanServerConnection mBeanServerConnection) {
									KafkaMetrics metrics = new KafkaMetrics();
									response.put("BytesInPerSec", new JSONObject(new FormatedMeterMetric(
											metrics.getBytesInPerSec(mBeanServerConnection, Optional.empty()))));
									response.put("BytesOutPerSec", new JSONObject(new FormatedMeterMetric(
											metrics.getBytesOutPerSec(mBeanServerConnection, Optional.empty()))));
									response.put("BytesRejectedPerSec", new JSONObject(new FormatedMeterMetric(
											metrics.getBytesRejectedPerSec(mBeanServerConnection, Optional.empty()))));
									response.put("FailedFetchRequestsPerSec",
											new JSONObject(new FormatedMeterMetric(metrics.getFailedFetchRequestsPerSec(
													mBeanServerConnection, Optional.empty()))));
									response.put("FailedProduceRequestsPerSec",
											new JSONObject(
													new FormatedMeterMetric(metrics.getFailedProduceRequestsPerSec(
															mBeanServerConnection, Optional.empty()))));
								}
							});
				}
				...
				kafkaJMX.doWithConnection(jmxArr[1], Integer.parseInt(jmxArr[2]), Optional.of(""), Optional.of(""),
						false, new JMXExecutor() {
							...
							public void doWithConnection(MBeanServerConnection mBeanServerConnection) {
								...
								if (result.containsKey("BytesInPerSec")) {
									result.put("BytesInPerSec", merge(result.get("BytesInPerSec"),
											metrics.getBytesInPerSec(mBeanServerConnection, Optional.of(topic))));
								} else {
									result.put("BytesInPerSec",
											metrics.getBytesInPerSec(mBeanServerConnection, Optional.of(topic)));
								}
								...
								if (result.containsKey("BytesOutPerSec")) {
									result.put("BytesOutPerSec", merge(result.get("BytesOutPerSec"),
											metrics.getBytesOutPerSec(mBeanServerConnection, Optional.of(topic))));
								} else {
									result.put("BytesOutPerSec",
											metrics.getBytesOutPerSec(mBeanServerConnection, Optional.of(topic)));
								}
								...
								if (result.containsKey("BytesRejectedPerSec")) {
									result.put("BytesRejectedPerSec", merge(result.get("BytesRejectedPerSec"),
											metrics.getBytesRejectedPerSec(mBeanServerConnection, Optional.of(topic))));
								} else {
									result.put("BytesRejectedPerSec",
											metrics.getBytesRejectedPerSec(mBeanServerConnection, Optional.of(topic)));
								}
								...
								if (result.containsKey("FailedFetchRequestsPerSec")) {
									...
									result.put("FailedFetchRequestsPerSec", metrics
											.getFailedFetchRequestsPerSec(mBeanServerConnection, Optional.of(topic)));
								}
								if (result.containsKey("FailedProduceRequestsPerSec")) {
									result.put("FailedProduceRequestsPerSec",
											merge(result.get("FailedProduceRequestsPerSec"),
									...
									result.put("FailedProduceRequestsPerSec", metrics
											.getFailedProduceRequestsPerSec(mBeanServerConnection, Optional.of(topic)));
								}
							}

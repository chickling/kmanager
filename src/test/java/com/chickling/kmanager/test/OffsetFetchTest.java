...
public class OffsetFetchTest {
  ...
  static {
    ...
    INCLUDE_GROUP.addAll(
        Arrays.asList("BI_EC_Pf_Newegg_com_adobe_products", "BI_EC_Pf_Newegg_com_adobe_others", "BI_EC_Pf_Newegg_com_adobe_allpage1",
            "I_EC_Pf_Newegg_com_adobe_middle_layer", "BI_EC_Pf_Newegg_com_adobe_hbaserawdata", "BI_EC_Pf_Newegg_com_adobe_COUNT"));
  }
  ...
  public static void main(String[] args) {
    ...
    builder.stream(Serdes.ByteArray(), Serdes.ByteArray(), OFFSET_TOPIC)
        // @formatter:off
        .map((k, v) -> new KeyValue<BaseKey, byte[]>(GroupMetadataManager.readMessageKey(ByteBuffer.wrap(k)), v)).filter((k, v) -> {
        ...
        .map((k, v) -> new KeyValue<OffsetKey, OffsetAndMetadata>((OffsetKey) k,
            /* v 有可能是 null */ v == null ? null : GroupMetadataManager.readOffsetMessageValue(ByteBuffer.wrap(v))))
        // .to(OUTPUT_TOPIC)
